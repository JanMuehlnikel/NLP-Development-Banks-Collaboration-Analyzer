...
def create_files():
    ...
    # CREATE CRS DF
    crs_df = df[["iati_id", "orga_abbreviation", "orga_full_name", "client", "title_main", "country", "description_main", "status", "crs_5_code", "crs_3_code"]]
    crs_df.to_csv("../../app/src/crs_data.csv", index=False)

    # CREATE SDG DF
    sdg_df = df[["iati_id", "iati_orga_id", "orga_abbreviation", "orga_full_name", "client", "title_main", "country", "description_main", "status", "sgd_pred_code"]]
