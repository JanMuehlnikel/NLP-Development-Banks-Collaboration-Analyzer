...
def create_files():
    ...
    orgas_df.to_csv("../../synergy_app/src/projects/project_orgas.csv", index=False)

    # Project Text (description & title)
    texts_df = df[["iati_id", "title_main", "description_main"]]
    texts_df.to_csv("../../synergy_app/src/projects/project_texts.csv", index=False)

    # Project Status DF
    status_df = df[["iati_id", "status", "planned_start", "actual_start","planned_end", "actual_end"]]
    status_df.to_csv("../../synergy_app/src/projects/project_status.csv", index=False)

    # Project region DF
    region_df = df[["iati_id", "country", "country_code_list", "country_name", "country_flag", "region", "location"]]
    region_df.to_csv("../../synergy_app/src/projects/project_region.csv", index=False)
    ...
    sector_df.to_csv("../../synergy_app/src/projects/project_sector.csv", index=False)
